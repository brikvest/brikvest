...
**Account Status Flow**: New users are auto-approved on signup (`accountStatus: 'approved'`) and auto-logged-in. Admin still receives a notification email for every new signup and can view all members in the admin dashboard. Legacy `pending` accounts are auto-promoted to `approved` on next login. The `rejected` status remains and blocks login.
...
**Shareable Listing Links**: Each resale listing gets a unique `shareToken` (24-char hex) on creation, stored in the `resale_listings` table. Live listings (approved/awaiting_payment/sold) are viewable by anyone at `/listing/:shareToken` via a public page that shows property details, pricing, bid count, and a CTA to sign in. The public API endpoint `GET /api/public/listing/:shareToken` returns sanitized listing data without requiring auth. Sellers see "Copy Link" and WhatsApp share buttons on live listings in their dashboard. All bids, purchases, and payments still require authentication and happen on-platform.
...
**Co-Investor Notifications**: When a resale listing is approved by admin and goes live, all confirmed investors (`converted_to_investment` status) in the same property are automatically notified via email — excluding the seller. Each co-investor receives a branded email with listing details (property name, units, type, price) and a direct link to the listing (using the shareable link if available). This creates a social network dynamic where co-owners get early visibility into secondary market activity. Notification failures are isolated per investor so one failure doesn't block others. Email function: `sendNewListingNotificationToCoInvestors()` in `server/resaleEmails.ts`.
