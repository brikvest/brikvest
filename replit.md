...
**P2P Unit Resale System**: Properties have an `isTransferable` boolean (default `false`) that admins toggle per-property via the admin property form. When enabled, investors holding confirmed investments can list their units for resale to other approved members. The field is stored in the `properties` table as `is_transferable`.
